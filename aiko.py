...
class arepo_reader():
    #defines vars
    def __init__(self, path, snapshotRange, particleTypes, blockNames):
        if path[-1] is not "/":
            path = path + "/"
        self.path = path
    ...
    def read_arepo(self):
        ...
        for snapshot in self.snapshots:
            currentPath = self.get_path(snapshot)
            currentInstance = snapshot_read(currentPath, particleTypes=self.particleTypes, blockNames=self.blockNames)
            currentData = currentInstance.read_arepo_snap()
            dataList[snapshot] = currentData
        print("processing...")
